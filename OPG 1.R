...
#######################################################################################################
#De 3 spillere for henholdsvis Viborg og Nordsjælland, med den højeste succesfulde afleveringsprocent. 
#######################################################################################################
#nordsjælland
fcn_player_pass_succes <- passes_succes_super %>% filter(TEAM_WYID==7458) %>% count(PLAYER_WYID) %>% arrange(desc(n))
colnames(fcn_player_pass_succes)[2] <- "succes"
fcn_player_pass_succes <- fcn_player_pass_succes %>% filter(succes>=150)
fcn_player_pass_total <- passes_super %>% filter(TEAM_WYID==7458) %>% count(PLAYER_WYID) %>% arrange(desc(n))
colnames(fcn_player_pass_total)[2] <- "total"
fcn_passes <- left_join(fcn_player_pass_succes, fcn_player_pass_total, by="PLAYER_WYID")
fcn_passes <- cbind(team=rep("FCN", nrow(fcn_passes)), fcn_passes, succesfuld_procentdel=(fcn_passes$succes/fcn_passes$total)*100)
fcn_passes <- fcn_passes %>% arrange(desc(succesfuld_procentdel))
fcn_passes <- left_join(fcn_passes, players[,c(2,3,4,13)] %>% filter(SEASON_WYID==191611), by="PLAYER_WYID")
fcn_passes <- fcn_passes %>% filter(complete.cases(.)) %>% slice(1:3)

#viborg
vff_player_pass_succes <- passes_succes_super %>% filter(TEAM_WYID==7456) %>% count(PLAYER_WYID) %>% arrange(desc(n))
colnames(vff_player_pass_succes)[2] <- "succes"
vff_player_pass_succes <- vff_player_pass_succes %>% filter(succes>=150)
vff_player_pass_total <- passes_super %>% filter(TEAM_WYID==7456) %>% count(PLAYER_WYID) %>% arrange(desc(n))
colnames(vff_player_pass_total)[2] <- "total"
vff_passes <- left_join(vff_player_pass_succes, vff_player_pass_total, by="PLAYER_WYID")
vff_passes <- cbind(team=rep("VFF", nrow(vff_passes)), vff_passes, succesfuld_procentdel=(vff_passes$succes/vff_passes$total)*100)
vff_passes <- vff_passes %>% arrange(desc(succesfuld_procentdel))
vff_passes <- left_join(vff_passes, players[,c(2,3,4,13)] %>% filter(SEASON_WYID==191611), by="PLAYER_WYID")
vff_passes <- vff_passes %>% filter(complete.cases(.)) %>% slice(1:6)


#join med player-table fra SQL
fcn_vff_passes <- rbind(vff_passes, fcn_passes)
fcn_vff_passes <- fcn_vff_passes %>% filter(SHORTNAME != "I. Näsberg", SHORTNAME != "L. Lund", SHORTNAME != "N. Bürgy")
team_colors <- setNames(c("#B3FFB3", "#4DCC4D", "#008000", "#FFB3B3", "#FF4D4D", "#B30000"), fcn_vff_passes$SHORTNAME)

#plot
ggplot(fcn_vff_passes) +
  geom_bar(
    aes(x = team, y = succesfuld_procentdel, fill = SHORTNAME), stat = "identity", position = "dodge") + 
  coord_cartesian(ylim = c(80, 100)) + scale_fill_manual(values = team_colors) +
    ggtitle("De 3 spillere for henholdsvis Viborg og \nNordsjælland, med den højeste succesfulde \nafleveringsprocent, er alle forsvarsspillere") +
    theme(plot.title = element_text(size = 30, face = "bold", color = "darkblue", 
                                      hjust = 0, vjust = 1.1, family = "serif"))
